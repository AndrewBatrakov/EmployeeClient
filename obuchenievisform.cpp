...
ObuchenieVisForm::ObuchenieVisForm(QString id, QWidget *parent, bool onlyForRead) : QDialog(parent)
{
    ...
    labelGruppaObuch = new QLabel(trUtf8("Группа №:"));
    editGruppaObuch = new LineEdit;

    QHBoxLayout *protocolBox = new QHBoxLayout;
    protocolBox->addWidget(editProtocolObuch);
    protocolBox->addWidget(labelGruppaObuch);
    protocolBox->addWidget(editGruppaObuch);
    protocolBox->addStretch();
    ...
    if(indexTemp != ""){
        QSqlQuery query;
        query.prepare("SELECT * FROM obuchvis WHERE obuchvisid = :id");
        ...
        while(query.next()){
            ...
            editGruppaObuch->setText(query.value(5).toString());
            editProtocolObuch->setText(query.value(6).toString());
            QSqlQuery queryID;
            queryID.prepare("SELECT * FROM obuchvistable WHERE obuchvisid = :obuchid");
    ...
    mainLayout->addLayout(protocolBox,3,1);
    ...
    setWindowTitle(trUtf8("Обучение (Высота)"));
    readSettings();
    createContextMenu();
    editNumber->setText(indexTemp);
...
void ObuchenieVisForm::done(int result)
{
    writeSettings();
    QDialog::done(result);
}

void ObuchenieVisForm::saveRecord()
{
    editRecord();
    emit accept();
}

void ObuchenieVisForm::cancelRecord()
{
    ...
    if(addIntoTable){
        ...
        for(int row = 0; row < employeeView->rowCount(); ++row){
            QSqlQuery queryCD;
            queryCD.prepare("DELETE FROM obuchvistable WHERE obuchvisid = :id");
            queryCD.bindValue(":id",indexTemp);
            queryCD.exec();
            if(!queryCD.isActive()){
                QMessageBox::warning(this,QObject::trUtf8("Obuc VIS Table, DELETE ERROR!"),queryCD.lastError().text());
                return;
            }
            line += "DELETE FROM obuchvistable WHERE obuchvisid = '";
...
void ObuchenieVisForm::editRecord()
{
    QTextStream stream(&exchangeFile);
    QString line;
    while(!stream.atEnd()){
    ...
    QSqlQuery queryIns;
    queryIns.prepare("SELECT * FROM obuchvis WHERE (obuchvisid = :id AND datedoc = :datedoc)");
    ...
    if(queryIns.isValid()){
        QSqlQuery query;
        query.prepare("UPDATE obuchvis SET "
                      "dogovor = :dogovor, "
                      "datedoc = :datedoc, "
                      "datecor = :datecor, "
                      "dateobuch = :dateobuch, "
                      ...
                      "WHERE obuchvisid = :obuchvisid");
        query.bindValue(":obuchvisid",indexTemp);
        ...
        query.bindValue(":gruppa",editGruppaObuch->text().simplified());
        query.bindValue(":numberbuch",editProtocolObuch->text().simplified());
        query.exec();
        line += "UPDATE obuchvis SET dogovor = '";
        ...
        line += "', ";
        line += "gruppa = '";
        line += editGruppaObuch->text().toUtf8();
        line += "', ";
        line += "numberbuch = '";
        line += editProtocolObuch->text().toUtf8();
        line += "' WHERE obuchvisid = '";
        line += indexTemp.toUtf8();
        line += "'";
        line += "\r\n";
        stream<<line;
        if(!query.isActive()){
            QMessageBox::warning(this,QObject::trUtf8("Obuch Vis, UPDATE ERROR!"),query.lastError().text());
            return;
        }
    }else{
        QSqlQuery queryInsert;
        queryInsert.prepare("INSERT INTO obuchvis ("
                            "obuchvisid, "
                            "dogovor, "
                            "datedoc, "
                            "datecor, "
                            ...
                            ":numberbuch"
                            ");");
        queryInsert.bindValue(":obuchvisid",indexTemp);
        ...
        queryInsert.bindValue(":gruppa",editGruppaObuch->text().simplified());
        queryInsert.bindValue(":numberbuch",editProtocolObuch->text().simplified());

        line += "INSERT INTO obuchvis (obuchvisid, dogovor, datedoc, datecor, "
                "dateobuch, gruppa, numberbuch) VALUES('";
        line += indexTemp.toUtf8();
        line += "', '";
        line += editDogovor->text().simplified().toUtf8();
        line += "', '";
        ...
        if(!queryInsert.isActive()){
            QMessageBox::warning(this,QObject::trUtf8("Obuch Vis, INSERT ERROR!"),queryInsert.lastError().text());
            return;
        }
    }
}
...
void ObuchenieVisForm::deleteRecord()
{
    QTextStream stream(&exchangeFile);
    QString line;
    while(!stream.atEnd()){
    ...
    settings.setValue("Table", "obuchvis");
    settings.setValue("Index",indexTemp);

    ForDelete forDelete(indexTemp,"obuchvis",this);
    ...
    if(k == QMessageBox::Yes){
        ...
        QSqlQuery query;
        query.prepare("DELETE FROM obuchvis WHERE obuchvisid = :obuchvisid");
        query.bindValue(":obuchvisid",indexTemp);
        query.exec();
        query.next();

        line += "DELETE FROM obuchvis WHERE obuchvisid = '";
        line += indexTemp.toUtf8();
        line += "'";
        line += "\r\n";
        stream<<line;
...
void ObuchenieVisForm::readSettings()
{
    QSettings settings("AO_Batrakov_Inc.", "EmployeeClient");
    restoreGeometry(settings.value("ObuchenieVisForm").toByteArray());
}

void ObuchenieVisForm::writeSettings()
{
    QSettings settings("AO_Batrakov_Inc.", "EmployeeClient");
    settings.setValue("ObuchenieVisForm", saveGeometry());
}

void ObuchenieVisForm::addRecordOfTable()
{
    if(checkingFill()){
        QTextStream stream(&exchangeFile);
        QString line;
        ...
        while(query.next()){
            ...
            QSqlQuery queryDoc;
            queryDoc.prepare("SELECT (SELECT o.dateobuch  FROM obuchvis AS o WHERE o.obuchvisid = ot.obuchvisid)"
                             "FROM obuchvistable AS ot WHERE ot.employeeid = :id");
            ...
            if(insert){
                ...
                NumPrefix numPrefix;
                QString idTable = numPrefix.getPrefix("obuchvistable");

                QSqlQuery queryPSL;
                queryPSL.prepare("INSERT INTO obuchvistable ("
                                "obuchvistableid, obuchvisid, employeeid"
                                ") VALUES(:obuchvistableid, :obuchvisid, :employeeid)");
                queryPSL.bindValue(":obuchvistableid",idTable);
                queryPSL.bindValue(":obuchvisid",indexTemp);
                queryPSL.bindValue(":employeeid",query.value(0).toString());
                queryPSL.exec();
                if(!queryPSL.isActive()){
                    QMessageBox::warning(this,QObject::trUtf8("Obuch Vis List Table, INSERT ERROR!"),queryPSL.lastError().text());
                    return;
                }
                line += "INSERT INTO obuchvistable (obuchvistableid, obuchvisid, employeeid) VALUES('";
                line += idTable.toUtf8();
                line += "', '";
                line += indexTemp.toUtf8();
                line += "', '";
...
void ObuchenieVisForm::deleteRecordOfTable()
{
    QTextStream stream(&exchangeFile);
    QString line;
    while(!stream.atEnd()){
    ...
    if(k == QMessageBox::Yes){
        QSqlQuery query;
        query.prepare("DELETE FROM obuchvistable WHERE (obuchvisid = :obuchvisid AND employeeid = :employeeid)");
        query.bindValue(":obuchvisid",indexTemp);
        query.bindValue(":employeeid",employeeView->item(employeeView->currentRow(),0)->text());
        query.exec();

        if(!query.isActive()){
            QMessageBox::warning(this,QObject::trUtf8("Obuch Vis Table, DELETE ERROR!"),
...
void ObuchenieVisForm::editRecordOfTable()
{
    QMessageBox::warning(this,"Attention!!!","You are not able to edit!");
}

bool ObuchenieVisForm::checkingFill()
{
    bool fill = true;
    if(editDogovor->text() == ""){
...
void ObuchenieVisForm::updateTable()
{

}

void ObuchenieVisForm::createContextMenu()
{
...
void ObuchenieVisForm::printTable()
{
...
int ObuchenieVisForm::printParagraph(QString paragraphString, QPainter *painter, QRect *rect, int spacing)
{
...
void ObuchenieVisForm::print(QPrinter *printer)
{
...
void ObuchenieVisForm::excelExport()
{
