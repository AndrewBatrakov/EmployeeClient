...
void XMLImport::traverseAllInformation(const QDomNode &node)
{
            ...
            if(!domElement.isNull()){
                if(domElement.tagName() == "Element"){
                    ...
                    if(queryPer.isValid()){
                        ...
                        QString ptmNumber;
                        ptmNumber = domElement.attribute("ptmUdDat","").simplified();
                        QString ptmData;
                        ptmData = domElement.attribute("ptmUdNumber","").simplified();
                        ...
                        if(allData){
                            ...
                            if(ptmPTMNumber != "" && ptmPTMDate != ""){
                                QSqlQuery queryPTM;
                                queryPTM.prepare("SELECT * FROM ptm WHERE (employeeid  = :id AND ptmnumber = :ptmnumber "
                                                 "AND ptmdate = :ptmdate);");
                                queryPTM.bindValue(":id",personId);
                                queryPTM.bindValue(":ptmdate",ptmPTMDate);
                                queryPTM.bindValue(":ptmnumber",ptmPTMNumber);
                                queryPTM.exec();
                                queryPTM.next();
                                if(!queryPTM.isValid()){
                                    QString ptmid;
                                    if(ptmPTMProgramma != ""){
                                        ...
                                        if(!queryO.isValid()){
                                            QSqlQuery query;
                                            ptmid = numPrefix.getPrefix("ptmprogramma");
                                            query.prepare("INSERT INTO ptmprogramma ("
                                                          "ptmprogrammaid, "
                                                          "ptmprogrammaname"
                                                          ") VALUES(:ptmprogrammaid, :ptmprogrammaname);");
                                            query.bindValue(":ptmprogrammaid",ptmid);
                                            query.bindValue(":ptmprogrammaname",ptmPTMProgramma);
                                            query.exec();
                                        }else{
                                            ptmid = queryO.value(0).toString();
                                        }
                                    }
                                    QSqlQuery query;
                                    QString idFire = numPrefix.getPrefix("ptm");
