...
PrichinaObuchForm::PrichinaObuchForm(QString id, QWidget *parent, bool onlyForRead) : QDialog(parent)
{
    ...
    setWindowTitle(tr("Prichina Obuch"));
    readSettings();
}
...
void PrichinaObuchForm::editRecord()
{
    ...
    if(indexTemp != ""){
        ...
        query.exec();
        if(!query.isActive()){
            QMessageBox::warning(this,QObject::tr("PrichinaObuch, UPDATE ERROR!"),query.lastError().text());
            return;
        }
        ...
        if(!queryInsert.isActive()){
            QMessageBox::warning(this,QObject::tr("Prichina Obuch, INSERT ERROR!"),queryInsert.lastError().text());
            return;
        }
