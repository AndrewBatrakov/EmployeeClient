    ...
    QDialog(parent)
{
    ...
    if(indexTemp != ""){
        ...
        if(!query.isActive()){
            QMessageBox::warning(this,QObject::trUtf8("Security Update ERROR!"),query.lastError().text());
        }
    }
    ...
    setWindowTitle(trUtf8("Security"));
...
void Security::editRecord()
{
    if(indexTemp != ""){
        ...
        if(!query.isValid()){
            ...
            query.exec();
            if(!query.isActive()){
                QMessageBox::warning(this,QObject::trUtf8("Security INSERT ERROR!"),query.lastError().text());
            }
        }else{
